...
if ( isset( $_GET['oauth_token'] ) || isset($_GET["oauth_verifier"]) ) {
	...
	// //ユーザーホームのURL
	$url = "https://twitter.com/".$query["screen_name"];
	...
	//2.mySQLから
	$stmt = $pdo->prepare("INSERT INTO user(id, access_token, access_token_secret, name, screen_name, user_id, icon, manage_flg, life_flg, indate)VALUES(NULL, :access_token, :access_token_secret, NULL, :screen_name, :user_id, :icon, 0, 1, sysdate())");
	$stmt -> bindValue(':access_token', $query["oauth_token"], PDO::PARAM_STR);
	$stmt -> bindValue(':access_token_secret', $query["oauth_token_secret"], PDO::PARAM_STR);
	$stmt -> bindValue(':screen_name', $query["screen_name"], PDO::PARAM_STR);
	$stmt -> bindValue(':user_id', $query["user_id"], PDO::PARAM_INT);
